...
// For Arabic, we translate metadata fields and reuse English HTML body.
// Long-form Arabic translations of bodies can be added incrementally.
// ============================================================
...
const arPosts: Record<string, PostAr> = {
  'saas-mvp-in-6-weeks': {
    title: 'كيف أُطلق منصة SaaS تجريبية في ٦ أسابيع بالضبط (وما الذي أحذفه)',
    excerpt: 'الستة أسابيع ليست موعداً نهائياً — إنها دالة إجبار. العمل الحقيقي هو تحديد ما لا تبنيه. هذه قائمتي الدقيقة بالميزات التي أحذفها كل مرة، والثلاث التي لا أتنازل عنها، ولماذا الأسبوع الخامس هو الأصعب.',
    category: 'المعمارية',
    thumbLabel: 'الإطلاق في ستة أسابيع.',
    tags: ['SaaS', 'المعمارية', 'منهجية', 'MVP', 'Laravel', 'مؤسسون'],
    lead: 'الستة أسابيع ليست موعداً نهائياً. إنها دالة إجبار. العمل الحقيقي ليس تحديد ما تبنيه — بل ما لا تبنيه. بعد إطلاق +٤٠ من هذه، إليك الإطار الدقيق، الميزات التي أحذفها في كل مرة، ولماذا الأسبوع الخامس هو الذي يكسر المؤسسين دائماً.',
    toc: [
      { id: 'skeleton', label: 'الهيكل من ستة أسابيع' },
      { id: 'cuts', label: 'أشياء أحذفها كل مرة' },
      { id: 'non-negotiables', label: 'الثلاث غير القابلة للتفاوض' },
      { id: 'week-five', label: 'لماذا الأسبوع الخامس هو الأصعب' },
      { id: 'launch', label: 'الأسبوع السادس — أسبوع الإطلاق' },
      { id: 'outside-mvp', label: 'ما خارج الـ MVP' },
      { id: 'truth', label: 'الحقيقة المزعجة' },
    ],
    faq: [
      { q: 'ماذا لو لم تكن الستة أسابيع كافية لمنتجي؟', a: 'إذاً ليست MVP — بل v1. يمكننا بالتأكيد بناءها، لكنني سأقدّر سعرها كارتباط ١٠–١٤ أسبوعاً. إطار الستة أسابيع يعمل فقط لمجموعة فرعية صارمة من نطاقات "أصغر منتج مفيد".' },
      { q: 'هل تضمن جدول الستة أسابيع؟', a: 'نعم، مع تحفظ: الجدول يفترض أنك متجاوب (ردود خلال ٢٤ ساعة)، النطاق محذوف بالفعل لحجم MVP، وليس هناك تحولات كبيرة في منتصف المشروع. إذا انزلق الجدول بسبب عملي، أكمل مجاناً حتى الإطلاق. إذا انزلق بسبب توسّع النطاق، نُعيد التسعير.' },
      { q: 'ما الـ stack الذي تستخدمه لـ SaaS MVPs؟', a: 'Laravel + Inertia + React في الخلفية والواجهة، Tailwind للتصميم، MySQL للبيانات، Redis للكاش وقوائم الانتظار، منشور عبر Forge أو Docker على بنيتك التحتية المختارة. تقليدي، مُجرَّب، سريع للإطلاق.' },
      { q: 'هل يمكنني رؤية مثال على MVP حقيقي مدته ستة أسابيع؟', a: 'معظمها تحت NDA، لكن UCOFY (في قسم الأعمال) بدأ كـ MVP ستة أسابيع لحالة استخدام واحدة قبل أن يتطور إلى المنصة الكاملة التي تراها الآن. النسخة الأولى أُطلقت في ٤١ يوماً.' },
      { q: 'كم تكلفة SaaS MVP؟', a: 'تبدأ من ٤,٥٠٠$ لـ MVP بميزة واحدة. معظم الارتباطات تقع بين ٥,٥٠٠$ و ٧,٥٠٠$ حسب التكاملات (Stripe، البريد المعاملاتي، APIs خارجية). سعر ثابت، لا مفاجآت.' },
    ],
  },
  'laravel-performance': {
    title: 'من ٨ ثواني إلى ٦٠٠ مللي ثانية: تشريح أداء Laravel',
    excerpt: 'كانت اللوحة بطيئة وكان العميل صبوراً — حتى لم يعد كذلك. إليك سجل التحليل الفعلي، الاختناقات الخمسة التي وجدناها، والإصلاحات التي حركت الإبرة فعلاً.',
    category: 'الأداء',
    thumbLabel: 'Laravel',
    tags: ['Laravel', 'الأداء', 'MySQL', 'تحليل'],
    lead: '٨.١ ثانية إلى ٦٠٠ مللي ثانية في ٤٨ ساعة. لا إعادة كتابة، لا بنية تحتية جديدة، لا تغييرات فريق. فقط خمسة إصلاحات جراحية بالترتيب الصحيح. الاستعلامات الفعلية، الأرقام الفعلية، الترتيب الفعلي للعمليات.',
    toc: [
      { id: 'baseline', label: 'الخطوة صفر: قِس أولاً' },
      { id: 'bottleneck-one', label: '#١: كذبة eager-loading' },
      { id: 'bottleneck-two', label: '#٢: ترقيم الصفحات في PHP' },
      { id: 'bottleneck-three', label: '#٣: JSON بدون فهرس' },
      { id: 'bottleneck-four', label: '#٤: الكاش الذي لم يكن' },
      { id: 'bottleneck-five', label: '#٥: إعادة الرسم في الواجهة' },
      { id: 'lessons', label: 'ما أتعلمه' },
    ],
    faq: [
      { q: 'هل تقدّم تحسين أداء مضموناً؟', a: 'نعم — تخفيض ٥٠٪ في تحميل الصفحة P95 أو استرداد المبلغ. عملياً كل ارتباط جاء أعلى بكثير من ذلك، لكن الضمان هو ما نلتزم به تعاقدياً.' },
      { q: 'كم يستغرق ارتباط أداء نموذجي؟', a: 'يومان إلى خمسة للتشخيص وأهم ثلاث إصلاحات. العمل المتبقي يعتمد على ما نجده — إعادة كتابة لمسار ساخن واحد عادةً أسبوع آخر، الضبط الكامل أسبوعان إلى ثلاثة.' },
      { q: 'هل ستحتاج وصولاً للإنتاج؟', a: 'أعمل من نسخة staging حديثة ببيانات مشابهة للإنتاج ووصول للقراءة فقط لسجلات الإنتاج. لا أُشغل migrations على قاعدة بياناتك الإنتاجية بدون مهندسك في الغرفة.' },
      { q: 'هل تعمل على Laravel فقط؟', a: 'لا — نفس التقنيات تنطبق على Node و Rails و Django. Laravel فقط حيث لديّ أعمق خبرة. أرسل لي ملاحظة عن stack الخاص بك.' },
    ],
  },
  'flutter-offline-sync': {
    title: 'Flutter يعمل دون اتصال: استراتيجية المزامنة التي تصمد فعلاً في الميدان',
    excerpt: 'السائقون يدخلون تحت الأرض. سائقو التوصيل يفقدون الإشارة لـ ٤٠ دقيقة. النهج الساذج "احفظ مؤقتاً وأعد المحاولة" يفسد البيانات. إليك نموذج حل التعارضات الذي أثق به.',
    category: 'Flutter',
    thumbLabel: 'Flutter',
    tags: ['Flutter', 'جوال', 'دون اتصال'],
    lead: 'السائقون يدخلون تحت الأرض. سائقو التوصيل يفقدون الإشارة لـ ٤٠ دقيقة. النهج الساذج للحفظ المؤقت وإعادة المحاولة يفسد البيانات. بعد ثلاث طبقات مزامنة إنتاجية — وإعادة كتابة مؤلمة واحدة — إليك نموذج حل التعارضات الذي أثق به في كل مشروع جوال الآن.',
    toc: [
      { id: 'model', label: 'النموذج الذهني' },
      { id: 'storage', label: 'التخزين: SQLite' },
      { id: 'conflicts', label: 'حل التعارضات' },
      { id: 'sync-worker', label: 'عامل المزامنة' },
      { id: 'clock-skew', label: 'انحراف الساعة حقيقي' },
      { id: 'ux', label: 'ما تعرضه الواجهة' },
      { id: 'testing', label: 'كيف أختبره' },
    ],
    faq: [
      { q: 'لماذا SQLite بدلاً من Hive أو Isar؟', a: 'المعاملات والفهارس والاستعلامات المعقدة. أحمال المزامنة تحتاج الثلاثة. Hive و Isar رائعتان للكاش البسيط مفتاح-قيمة؛ SQLite الأداة الصحيحة عندما تحتاج "أعطني كل الأحداث غير المتزامنة من الجهاز X الأقدم من ٥ دقائق" بسرعة.' },
      { q: 'كم يصبح حجم قاعدة البيانات المحلية؟', a: 'على جهاز سائق توصيل بـ ١,٠٠٠ حدث/يوم، جدول الأحداث يبقى تحت ١٠ ميجا إذا حذفت الأحداث المتزامنة بعد ٧ أيام. جدول الحالة هو ما يحتاجه التطبيق — عادة بضع مئات من الكيلوبايت.' },
      { q: 'ماذا عن التشفير من طرف لطرف؟', a: 'أستخدم افتراضياً تخزين المنصة الآمن (Keychain / Keystore) للمفاتيح و SQLCipher لقاعدة البيانات على التطبيقات التي تتعامل مع بيانات حساسة. معظم تطبيقات الخدمات اللوجستية لا تحتاجه؛ الرعاية الصحية والمالية تحتاج.' },
      { q: 'هل يعمل هذا للتطبيقات التعاونية (نمط Google Docs)؟', a: 'مشكلة مختلفة. Last-write-wins لا يناسب هناك — تحتاج CRDTs أو operational transforms. النمط في هذه المقالة لتدفقات الأحداث المعاملاتية، وهي معظم تطبيقات الخدمات اللوجستية والعمليات والخدمة الميدانية.' },
    ],
  },
  'leading-eleven-engineers': {
    title: 'قيادة أحد عشر مهندساً دون التحول إلى مدير مشاريع',
    excerpt: 'لا أزال أكتب كوداً كل يوم. إليك كيف أنظّم أسبوعي، أحمي وقت الصناعة، ومع ذلك أراجع كل PR مهم — دون أن أصبح الاختناق.',
    category: 'القيادة',
    thumbLabel: 'الفريق',
    tags: ['القيادة', 'إدارة الهندسة'],
    lead: 'أقود فريقاً هندسياً من ١١ شخصاً ولا أزال أكتب كوداً كل يوم. ممكن بالكاد، معركة يومية ضد الانجراف — لكنه يعمل لأنني حذفت أشياء تبدو كالوظيفة لكنها ليست كذلك. إليك التقويم والطقوس وفلتر التوظيف الذي يحافظ على النصفين أمناء.',
    toc: [
      { id: 'rule-one', label: 'وقت الصناعة مقدّس' },
      { id: 'standup', label: 'الـ standup الذي لا يضيع الوقت' },
      { id: 'prs', label: 'مراجعات PR التي تهم' },
      { id: 'onones', label: 'جلسات ١-على-١، أسبوعياً' },
      { id: 'hiring', label: 'فلتر التوظيف' },
      { id: 'delegation', label: 'التفويض ليس إلقاءً' },
      { id: 'energy', label: 'حماية الطاقة' },
      { id: 'code', label: 'لماذا لا أزال أكتب كوداً' },
    ],
    faq: [
      { q: 'ألا يشعر الفريق بالإهمال عندما لا تراجع كل PR؟', a: 'سألتهم — العكس. يشعرون بالثقة. الخطر هو الفئات الأربع التي ذكرتها (المصادقة، migrations، APIs العامة، المعمارية المُعلَّمة)؛ في كل مكان آخر، مراجعة الأقران من اثنين كبار أسرع وجيدة بنفس القدر.' },
      { q: 'كم كوداً تشحن فعلياً في الأسبوع؟', a: 'تقريباً ٨–١٥ ساعة من البرمجة المركزة، عادة ٢–٤ PRs بحجم ذي معنى. أحياناً أكثر في sprint حيث آخذ refactor صعب؛ أحياناً أقل عندما يكون موسم التوظيف.' },
      { q: 'ماذا عن الثقافة وحملات التوظيف والأماكن الخارجية — هل تتخطاها؟', a: 'لا، لكنها تذهب في كتلة بعد الظهر، وليس الصباح. قاعدة ٠٩:٠٠–١٣:٠٠ ليست "لا تكن مديراً" — إنها "لا تكن مديراً قبل الغداء."' },
    ],
  },
  'multi-tenant-laravel': {
    title: 'Laravel متعدد المستأجرين: النمط الذي سأستخدمه في كل مرة',
    excerpt: 'عزل على مستوى الصف، schema مشترك، صفر كتابات مزدوجة. خمس سنوات من أخطاء multi-tenant مكثَّفة في ملف migration واحد و middleware.',
    category: 'المعمارية',
    thumbLabel: 'المعمارية',
    tags: ['Laravel', 'المعمارية', 'Multi-tenant'],
    lead: 'عزل على مستوى الصف، schema مشترك، صفر كتابات مزدوجة. خمس سنوات من أخطاء multi-tenant مكثَّفة في migration واحد، middleware واحد، global scope واحد. النمط الذي سأشحنه في اليوم الأول من كل Laravel SaaS — والمخارج التي تمنع أسوأ خطأ في النوع.',
    toc: [
      { id: 'why-shared', label: 'لماذا schema مشترك' },
      { id: 'the-pattern', label: 'النمط' },
      { id: 'resolution', label: 'حل المستأجر' },
      { id: 'global-scope', label: 'الـ global scope' },
      { id: 'jobs-and-mail', label: 'المهام والبريد' },
      { id: 'cross-tenant', label: 'استعلامات عبر المستأجرين' },
      { id: 'domain-per-tenant', label: 'نطاقات مخصصة' },
      { id: 'files', label: 'تخزين الملفات' },
      { id: 'closing', label: 'ما سأشحنه اليوم' },
    ],
    faq: [
      { q: 'ماذا عن القطاعات المنظَّمة التي تتطلب فصلاً فيزيائياً للبيانات؟', a: 'مشكلة مختلفة. إذا كان عقدك يتطلب مفاتيح تشفير لكل عميل أو إقامة بيانات حسب الولاية القضائية، schema المشترك خاطئ؛ تحتاج قاعدة بيانات لكل مستأجر أو حتى cluster لكل مستأجر. لـ ٩٥٪ من B2B SaaS، ليس هذا هو الحال.' },
      { q: 'Spatie/laravel-multitenancy أم tenancy/tenancy v3؟', a: 'Spatie. مساحة سطح أصغر، أسهل للتفكير فيه، والمشرف متجاوب. Tenancy v3 أكثر مرونة لكن حبل أكثر لشنق نفسك به.' },
      { q: 'هل يمكنني ترحيل تطبيق single-tenant لاحقاً؟', a: 'نعم، وقد فعلت ذلك مرتين. خصص ميزانية لستة أشهر. إنه في الغالب ترحيل بيانات آمن، تركيب الـ global scope، وإيجاد كل مكان افترض فيه مطور أن هناك مستأجر واحد فقط. لا تؤجله.' },
    ],
  },
  'ucofy-iot-ingest': {
    title: 'كيف يستقبل UCOFY ٥٨ ألف حدث IoT يومياً دون إسقاط واحد',
    excerpt: 'MQTT في cluster من queue workers، كتابات idempotent، واستراتيجية dead-letter تُقرأ فعلاً. المعمارية المملة التي جعلت التدقيق التنظيمي ممكناً.',
    category: 'المعمارية',
    thumbLabel: 'IoT',
    tags: ['IoT', 'المعمارية', 'MQTT', 'Laravel'],
    lead: '٥٨,٠٠٠ حدث يومياً من حساسات إدارة النفايات، الجهات التنظيمية تراقب، SLA تعاقدي يقول إننا لا نُسقط. بعد ٩٠ يوماً: ٥.١٨ مليون حدث مُستقبَل، صفر مُسقَط، ٩٩.٩٧٪ وقت تشغيل. المعمارية ليست ذكية — إنها النسخة المملة، مع إزالة كل مكان شاهدت فيه نسخة ذكية تفشل.',
    toc: [
      { id: 'problem', label: 'شكل المشكلة' },
      { id: 'topology', label: 'الطوبولوجيا' },
      { id: 'idempotent', label: 'كتابات Idempotent' },
      { id: 'dead-letter', label: 'الـ DLQ الذي يُقرأ' },
      { id: 'backpressure', label: 'الضغط العكسي' },
      { id: 'observability', label: 'ما نراقبه' },
      { id: 'audit', label: 'زاوية التدقيق' },
      { id: 'numbers', label: 'الأرقام الفعلية' },
    ],
    faq: [
      { q: 'لماذا MQTT وليس HTTP؟', a: 'الأجهزة على اتصالات خلوية تنقطع وتعيد الاتصال باستمرار. MQTT يحتفظ بجلسة طويلة الأمد ويستأنف بثمن بخس؛ HTTP سيحرق TCP handshake و TLS negotiation لكل حدث. عند ٥٨ ألف/يوم من مئات الأجهزة، هذه الرياضيات مهمة.' },
      { q: 'لماذا Go للمستهلك و Laravel للعمال؟', a: 'Go رائع في الإدخال/الإخراج عالي الإنتاج وقليل التأخير — مثالي لـ MQTT bridge. Laravel رائع لمنطق الأعمال والتحقق وقوائم الانتظار وواجهة الإدارة. كل أداة حيث تتألق، تتحدث عبر Redis.' },
      { q: 'هل يمكن أن يتسع هذا لـ ١٠× الحمل الحالي؟', a: 'نعم. الاختناق عند ٥٨٠ ألف/يوم سيكون كتابات MySQL، التي سنعالجها بـ horizontal sharding بـ device_id. معمارية الـ pipeline نفسها لا تتغير.' },
      { q: 'كيف تبدو واجهة dead-letter؟', a: 'صفحة إدارة بسيطة: قائمة الأحداث الفاشلة، الحمولة المُحلَّلة، تتبع الخطأ، الجهاز الذي أنتجها، وزر إعادة تشغيل. نستخدمها مرتين أسبوعياً — عادة بعد أن نشحن إصلاحاً لـ bug في parser.' },
    ],
  },
  'pair-programming-interviews': {
    title: 'الـ take-home الذي تخليت عنه — وما أسأل به المرشحين بدلاً عنه',
    excerpt: 'الـ take-homes تُرشّح المهارة الخاطئة. إليك صيغة البرمجة الثنائية لـ ٦٠ دقيقة التي استخدمتها في آخر ٣٠ توظيفاً، ولماذا تعطيني إشارة أفضل بألم أقل للمرشح.',
    category: 'القيادة',
    thumbLabel: 'التوظيف',
    tags: ['التوظيف', 'القيادة'],
    lead: 'كنت أرسل للمرشحين take-home. ثم لاحظت أن الـ take-home يُرشّح المرشحين العاطلين عن العمل ذوي السبت الفارغ. آخر ٣٠ توظيفاً جاءت عبر جلسة برمجة ثنائية ٦٠ دقيقة على bug حقيقي، ومعدل الإصابة تضاعف تقريباً.',
    toc: [
      { id: 'problem-with-takehome', label: 'ما تُرشّحه الـ take-homes' },
      { id: 'format', label: 'الصيغة التي أستخدمها' },
      { id: 'setup', label: 'الـ ٥ دقائق قبل' },
      { id: 'what-im-watching', label: 'ما أراقبه' },
      { id: 'rubric', label: 'المعيار، مكتوب' },
      { id: 'objections', label: 'الاعتراضان اللذان أحصل عليهما' },
      { id: 'results', label: 'النتائج' },
    ],
    faq: [
      { q: 'هل تخبر المرشحين بالـ bug مسبقاً؟', a: 'أخبرهم بالملف، وصف فقرة واحدة، والعرَض. لا أُري الإصلاح أو الـ diff — عمل تكوين فرضية هو نصف الإشارة.' },
      { q: 'ماذا لو كان المرشح متوتراً جداً للأداء؟', a: 'أتوقع التوتر. أفتح بـ ٥ دقائق من حديث منخفض المخاطر، أسرد تفكيري الخاص عندما أتنقل، وأقول صراحة إن أدوات AI و Google جيدة. المعيار يقيّم كيف يعملون، لا كيف يبدون مرتاحين.' },
      { q: 'كيف تمنع المعيار من الانجراف عبر المُقيّمين؟', a: 'نفس الخمسة أسطر، نفس قالب Notion، كل مرة. بعد كل مقابلة أكتب فقرة واحدة من الأدلة لكل سطر. الانجراف يأتي في الغالب من المعايير المفقودة، لا من المكسورة.' },
      { q: 'ماذا عن المرشحين من بلدان حيث البرمجة الثنائية غير شائعة؟', a: 'أشرح الصيغة في بريد الجدولة ومرة أخرى في البداية. حوالي ١ من ٦ مرشحين لم يبرمج بشكل ثنائي من قبل؛ أول ١٠ دقائق لهم أبطأ، لكن الإشارة تتعادل.' },
    ],
  },
  'flutter-stopped-bloc': {
    title: 'لماذا توقفت عن استخدام Bloc (وما حلّ محلها)',
    excerpt: 'Bloc كانت رائعة، حتى لم تعد كذلك. على آخر ثلاثة تطبيقات إنتاجية لدينا، تقلّصت بصمة إدارة الحالة ٦٠٪ بعد التحول إلى Riverpod + طبقة domain رفيعة.',
    category: 'Flutter',
    thumbLabel: 'جوال',
    tags: ['Flutter', 'Riverpod', 'إدارة الحالة'],
    lead: 'شحنت أربعة تطبيقات Flutter إنتاجية بـ Bloc، علّمتها لمبتدئَين، كتبت دليل أسلوب للفريق. ثم على آخر ثلاثة تطبيقات تحولت إلى Riverpod مع طبقة Controller رفيعة — وتقلّصت قاعدة الكود ٦٠٪. إليك ما توقّف عن العمل، وما حلّ محله.',
    toc: [
      { id: 'what-bloc-was-good-at', label: 'ما كانت Bloc جيدة فيه' },
      { id: 'what-stopped-working', label: 'ما توقّف عن العمل' },
      { id: 'thin-domain', label: 'طبقة domain الرفيعة' },
      { id: 'what-i-miss', label: 'ما أفتقده في Bloc' },
      { id: 'numbers', label: 'الأرقام' },
      { id: 'should-you', label: 'هل يجب أن تتحول؟' },
    ],
    faq: [
      { q: 'Riverpod أم Provider؟', a: 'Riverpod. Provider كان حجراً للوصول؛ Riverpod أصلح الأشياء التي أخطأ فيها Provider (سلامة وقت الترجمة، عدم الاعتماد على BuildContext، اختبار أفضل). مؤلف الاثنين يقول استخدم Riverpod — أتفق.' },
      { q: 'أليست Bloc أكثر قابلية للاختبار؟', a: 'بالكاد. Bloc تعطيك اختبار events-in-states-out، وهو نظيف. Controllers + Riverpod تعطيك instantiate-call-method-assert، وهو أكثر مباشرة. كلاهما جيدان؛ الفرق هو الذوق.' },
      { q: 'ماذا عن GetX؟', a: 'لن أستخدمها في مشروع جماعي. تشجّع الحالة العامة، تُخفي التبعيات، والتوثيق غير موثوق. المطورون المبتدئون يحبونها لأنها سريعة للبدء؛ الكبار يكرهونها لأنها صعبة الصيانة.' },
      { q: 'هل ستُرحّل تطبيق Bloc الموجود لدينا؟', a: 'أحياناً يستحق، غالباً لا. إذا كان الفريق سعيداً ويشحن، تكلفة الترحيل نادراً ما تستحق. إذا كان الفريق يحترق على boilerplate، عادة تستحق. أرسل لي ملاحظة بحجم قاعدة الكود وسأعطيك إجابة صريحة.' },
    ],
  },
  'arabic-rtl-patterns': {
    title: 'تطبيقات ويب ثنائية اللغة عربي/إنجليزي لا تعاني',
    excerpt: 'تنفيذ RTL صحيح ليس قلباً — إنه قرار تصميم مدمج من اليوم الأول. الأنماط الخمسة التي أستخدمها في كل مشروع عربي.',
    category: 'المعمارية',
    thumbLabel: 'RTL',
    tags: ['RTL', 'العربية', 'i18n'],
    lead: 'ست سنوات من تطبيقات ويب ثنائية اللغة عربي-إنجليزي. الأول استغرق ستة أشهر وأعدت التخطيط مرتين. الأحدث يضيف أسبوعاً من العمل لبناء إنجليزي فقط. الفرق ليس الموهبة — إنه خمسة أنماط، مدمجة من اليوم الأول. ركّبها في النهاية وستعاني.',
    toc: [
      { id: 'not-flip', label: 'RTL ليس "اقلب التخطيط"' },
      { id: 'pattern-1', label: 'الخصائص المنطقية' },
      { id: 'pattern-2', label: 'تصنيف الأيقونات' },
      { id: 'pattern-3', label: 'أزواج الطباعة' },
      { id: 'pattern-4', label: 'الأرقام تبقى LTR' },
      { id: 'pattern-5', label: 'i18n هي شجرة قرار' },
      { id: 'testing', label: 'كيف أختبر' },
      { id: 'closing', label: 'التكلفة، بصدق' },
    ],
    faq: [
      { q: 'هل أحتاج بناء منفصل للعربية؟', a: 'لا. قاعدة كود واحدة، بناء واحد، حزمة واحدة. خاصية dir على وسم html تقلب التطبيق بأكمله في وقت التشغيل. الأنماط الخمسة في هذه المقالة تجعل القلب آمناً.' },
      { q: 'ماذا عن العبرية والأردية والفارسية؟', a: 'نفس الأنماط. العبرية الأقرب للعربية في سلوك التخطيط. الأردية والفارسية تشتركان في عائلة الخط لكن لديها تفضيلات طباعية خاصة باللغة — تحدث مع متحدث أصلي عن اختيار الخط.' },
      { q: 'Tailwind أم vanilla CSS؟', a: 'Tailwind 3.3+ لديه أدوات الخصائص المنطقية (ms-*, me-*, ps-*, pe-*) التي تربط مباشرة بـ CSS في هذه المقالة. استخدمها. إذا كنت على Tailwind أقدم، حدّث أو ارجع إلى plugin مخصص صغير.' },
      { q: 'كيف أُجري QA على النسخة العربية؟', a: 'متحدث عربي أصلي، كل إصدار، حتى لو كان مرورة ٣٠ دقيقة. أخصص التكلفة. الانحدار البصري الآلي يلتقط أخطاء التخطيط؛ فقط متحدث أصلي يلتقط القضايا الطباعية وقضايا النبرة.' },
    ],
  },
}

const arPostCategories: Record<string, string> = {
...
export function getBlogPosts(locale: string): BlogPostMeta[] {
  if (locale !== 'ar') return blogPosts
  return blogPosts.map(p => {
    const a = arPosts[p.slug]
    if (!a) return p
    return {
      ...p,
      title: a.title,
      excerpt: a.excerpt,
      category: a.category || arPostCategories[p.category] || p.category,
      thumbLabel: a.thumbLabel,
      tags: a.tags,
      readTime: p.readTime.replace('min read', 'دقيقة قراءة'),
    }
  })
}

export function getBlogPostBodies(locale: string): Record<string, BlogPostFull> {
  if (locale !== 'ar') return blogPostBodies
  const out: Record<string, BlogPostFull> = {}
  for (const slug of Object.keys(blogPostBodies)) {
    const orig = blogPostBodies[slug]
    const a = arPosts[slug]
    if (!a) {
      out[slug] = orig
      continue
    }
    out[slug] = {
      ...
      title: a.title,
      excerpt: a.excerpt,
      category: a.category || arPostCategories[orig.category] || orig.category,
      thumbLabel: a.thumbLabel,
      tags: a.tags,
      readTime: orig.readTime.replace('min read', 'دقيقة قراءة'),
      lead: a.lead || orig.lead,
      toc: a.toc || orig.toc,
      faq: a.faq || orig.faq,
      // Long-form HTML body kept as English for now
      body: orig.body,
    }
  }
  return out
